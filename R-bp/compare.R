...
simu.compare <- function(S, tK, b, a, Unif.min, Unif.max, CD, Exp.r, logm, logs, minc, maxc, x1.u, x2.u, x1.s, x2.s, v.sd){
  ...
  ####### RUN
  {
    ...
    
    {
      ...
      p.v1  <- dataSt.p$v_sen
      p.v2  <- dataSt.p$v_spe 
      p.v3  <- dataSt.p$v_lnHR
      
      p.v12 <- dataSt.p$v_senspe 
      p.v13 <- dataSt.p$v_senlnHR 
      p.v23 <- dataSt.p$v_spelnHR 
      ...
      o.v1  <- dataSt.o$v_sen
      o.v2  <- dataSt.o$v_spe 
      o.v3  <- dataSt.o$v_lnHR
      
      o.v12 <- dataSt.o$v_senspe 
      o.v13 <- dataSt.o$v_senlnHR 
      o.v23 <- dataSt.o$v_spelnHR 
    }
    ...
    ## 1. BNM.ML ----
    {
      ...
      bnm.int    <- c(mean(p.y1), mean(p.y2), mean(p.v1), mean(p.v2), mean(p.v12))
      fn.bnm.ml  <- function(par) .llk.BNM.ml(par, p.y1, p.y2, p.v1, p.v2, p.v12)
