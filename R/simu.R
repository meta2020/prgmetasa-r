...
simu <- function(S, tK, b, a, N.min, N.max, CD, Exp.r, meanlog, sdlog, minc, maxc, x1.u, x2.u, x1.s, x2.s, v.sd, true.p=NULL){

  # S <- 50;b <- b0;a <- alpha;eps <- sqrt(.Machine$double.eps);CD <- "EXP";Exp.r <- 0.2;tK <- 2
  # true.p <- NULL
  ...
  ####### RUN
  {
    ...
    
    {
      ...
      if(!inherits(fit.tnm_p, "try-error")) {
        ...
        tnm.p[c(1:9, 15,16)]  <- c(fit.tnm_p$par, sauc, conv)
    ...
    
    {
      ...
      if(!inherits(fit.tnm_o, "try-error")) {
        ...
        tnm.o[c(1:9, 15,16)]  <- c(fit.tnm_o$par, sauc, conv)
    ...
    
    {
      ...
      # fn.tnm.ml  <- function(par) clk.TNM.ml(c(par, 1), o.y1, o.y2, o.y3, o.s11, o.s22, o.s33, o.s12, o.s13, o.s23, p=p.hat, a.interval = c(-10, 10))
      # 
      # fit.tnm_sa <- nlminb(
      #   c(tnm.int), fn.tnm.ml,
      #   lower = c(rep(-Inf,3), rep(eps, 3), rep(-1,3)),
      ...
      if(!inherits(fit.tnm_sa, "try-error")) {
        ...
        a.p <- function(a) {sum(1/f.b(a), na.rm = TRUE) - NN/p.hat}
        
        a.opt.try <- suppressWarnings(try(uniroot(a.p, c(-10,10), extendInt="no"), silent = TRUE)) 
        
        alpha <- a.opt.try$root
        
        p.est <- mean(pnorm(alpha+b*(t.p)))
        
        
        
        tnm.sa[c(1:11, 14:16)] <- c(fit.tnm_sa$par, alpha, p.est, sauc, conv)
