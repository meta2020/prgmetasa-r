...
simu <- function(S, tK, b, a, N.min, N.max, CD, Exp.r, logm, logs, minc, maxc, x1.u, x2.u, x1.s, x2.s, v.sd){
  ...
  # files.sources <- list.files(path = "../R2/"); x <- sapply(paste0("../R2/", files.sources), source); library(survival); library(mixmeta)
  # load(sprintf("../simulation-results/exp0.2/a-calc/beta_1_alpha_20.RData"))
  ...
  ####### RUN
  {
    ...
    
    {
      ...
      if(!inherits(fit.tnm_o, "try-error")) {
        ...
        if(fit.tnm_o$converged){ 
          ...
          tnm.int <- c(mu = unname(u), t=c(rep(0.1,6)))
          }
    ...
    {
      fn.tnm.cho  <- function(par) .clk.TNM.cho(par, o.y1, o.y2, o.y3, o.s11, o.s22, o.s33, o.s12, o.s13, o.s23, p=p.hat, a.interval = c(-10, 10))
      fit.tnm_sa <- nlminb(
        c(tnm.int, b0), fn.tnm.cho,
        lower = c(rep(-Inf,3), rep(-Inf, 6), 0),
        upper = c(rep( Inf,3), rep(Inf, 6), 2) )
      ...
      if(!inherits(fit.tnm_sa, "try-error")) {
        ...
        if(conv==0){
        	...
        	u <- fit.tnm_sa$par[1:3]
        	
        	L <- diag(rep(0,3))
        	L[lower.tri(L, diag = TRUE)] <- fit.tnm_sa$par[c(4:9)]
        	Omega <- L %*% t(L)
         ...
         tnm.sa <- c(u, t123, r1, r2, r3, b, sauc, prop, conv)
