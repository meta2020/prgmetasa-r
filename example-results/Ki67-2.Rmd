...
par(mfrow = c(1,2))
res <- rma(yi=u_lnHR, vi=v_lnHR, data=na.omit(dataHRALL[dataHRALL$cutoff==0.1 & dataHRALL$D==1,]), slab=author)
res.tf <- trimfill(res)
...
res <- rma(yi=u_lnHR, vi=v_lnHR, data=na.omit(dataHRALL[dataHRALL$cutoff==0.2 & dataHRALL$D==1,]), slab=author)
res.tf <- trimfill(res)
funnel(res.tf, cex=1.2, xlim=c(-2,3), legend = FALSE, xlab = "lnHR")
title("Cutoff = 20%")
...
TB1 <- t(round(res$par.all, 3)[c(1:11),c(1,5,7,9)])
