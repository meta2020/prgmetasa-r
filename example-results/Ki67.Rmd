...
init3 <- c(median(y1), median(y2), median(y3), 
           median(v1), median(v2), median(v3), 
           median(v12), median(v13), median(v23))
...
if (saveplot) {setEPS(width = 12, height = 6); postscript("ki67-sauc.eps")}
...
```{r, fig.height=6, fig.width=12}
if (saveplot) {setEPS(width = 12, height = 6); postscript("ki67-sroc.eps")}
...
# ytext <- TeX("$p = \\Phi(\\beta \\, t + \\alpha)$", output = "expression")
title.cex <- 1.2
...
if(saveplot) {setEPS(width = 12, height = 6); postscript("PROBIT.eps")}
...
curve(pnorm(beta1*x + alpha1), -10, 15, ylim = c(0,1),
      xlab = "", ylab = "",
      col=1,
      yaxt='n')
curve(pnorm(beta2*x + alpha2), -10, 15, add = TRUE, col=2) 
curve(pnorm(beta3*x + alpha3), -10, 15, add = TRUE, col=3)
