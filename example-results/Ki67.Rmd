...
t0 <- dataHRALL$t_lnHR[dataHRALL$D==0]
...
par.mvm.3 <- cbind(par.bnm.ml, PAR.3[c(1,2,4,5,7), c(3,5,7)])
ab.3 <- PAR.3[c(10,11),c(3,5,7)] 
...
sauc.mat.5 <- cbind(c(sauc.bnm, conv = opt.bnm.ml$convergence), PAR.5[c(12:14, 17),])
...
par.mvm.5 <- cbind(par.bnm.ml, PAR.5[c(1,2,4,5,7), c(3,5,7)])
ab.5 <- PAR.5[c(10,11),c(3,5,7)] 
...
## SROC
SROC(par = par.mvm.3, sroc.type = "sroc", spoint.cex = 2)
points(plogis(-data.3$u_spe), plogis(data.3$u_sen))
legend("bottomright", 
       bty='n',
       legend = c(sprintf("HZ, SAUC = %.3f", sauc.mat.3[1,1]), 
       											sprintf("p = %.2f, SAUC = %.3f", c(0.7, 0.5, 0.3), c(sauc.mat.3[1,c(4,6,8)]))), 
...
SROC(par = par.mvm.5, sroc.type = "sroc", spoint.cex = 2)
points(plogis(-data.5$u_spe), plogis(data.5$u_sen))
legend("bottomright", 
       bty='n',
       legend = c(sprintf("HZ, SAUC = %.3f", sauc.mat.5[1,1]), 
       											sprintf("p = %.2f, SAUC = %.3f", c(0.7, 0.5, 0.3), c(sauc.mat.5[1,c(4,6,8)]))), 
...
points(x, rep(1,   length(x)), pch="|", col=1, cex = 1)
points(t0, rep(0,   length(t0)), pch="|", col="darkgrey", cex = 1)
title("t = 3 (year)")
legend("bottomright", 
       bty='n',
