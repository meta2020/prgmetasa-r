tdsa<-function(
    HR.data, OS.data, MCT.data,
    tK=5, study=study, t=t, n1=n1, n0=n0, s1=s1, s0=s0, HR=HR, ci.low=ci.low,ci.up=ci.up,
    n1.mct = n1,n0.mct=n0,s1.med.mct=s1_med,s0.med.mct=s0_med,med.year = med.year,
    eta.range = c(0,1), init.eta = 0.01,
    sauc.type = "sroc"
){
	
  mct<- data.frame(study = eval(substitute(study), MCT.data),
                   n1 = eval(substitute(n1.mct), MCT.data),
                   n0 = eval(substitute(n0.mct), MCT.data),
                   s1 = eval(substitute(s1.med.mct), MCT.data),
                   s0 = eval(substitute(s0.med.mct), MCT.data),
                   med.year=eval(substitute(med.year),MCT.data)
  )
    eta <- cens.eta(data = mct, med.year = med.year,  n1 = n1, n0 = n0, s1_med = s1, s0_med = s0)$par

    data <- merge(OS.data, HR.data, all.x = TRUE)
    dt.os <- data.frame(study = eval(substitute(study), data),
                            ...
                            u_lnHR = log(eval(substitute(HR), data)),
                            v_lnHR = with(data,((log(ci.up)-log(ci.low))/(2 * qnorm(0.975)))^2)
        )
...
tdsa(HR.data=dataHR, 
					...
					n0=n0, 
					s1=s1, 
					s0=s0, HR=HR, ci.low=ci.low,ci.up=ci.up,
					n1.mct = n1,n0.mct=n0,s1.med.mct=s1_mct,s0.med.mct=s0_mct,med.year = mty,
					eta.range = c(0,1), init.eta = 0.01,
					sauc.type = "sroc")
