...
.table.sauc <- function(Sn=1,npt.n=1,p=7){
  ...
  med <- apply(DATA, 1:2, function(x) median(x, na.rm = TRUE))[12,]*100
  q1  <- apply(DATA, 1:2, function(x) quantile(x, probs = 0.25, na.rm = TRUE))[12,]*100
  q3  <- apply(DATA, 1:2, function(x) quantile(x, probs = 0.75, na.rm = TRUE))[12,]*100
  conv<- 1-apply(DATA, 1:2, function(x) mean(x, na.rm = TRUE))[11,5]
  ...
  res = rbind(res, c(s, mark.n, sauc, round(conv*100,2), 
                     sprintf("%.2f", (med[3]-med[1])), sprintf("%.2f", (med[5]-med[1]))
                     ))
  }
  
  
  colnames(res) <- c("$S (N)$","B",
                     "BNM$_P$", "TNM$_P$",
                     "BNM$_O$", "TNM$_O$",
                     "Proposal", "CR", "RB", "Bias")
...
npt = c("50-150", rep("", 17), "40-300", rep("", 17))
...
colnames(atab) = paste0("Biomarker",1:6)
