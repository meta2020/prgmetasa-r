...
result_list <- function(path = "exp0.2/N20/ptdis2_HR1_p0.8.RData", 
  ...
  cr.sum  <- round(100*matrix(sapply(1:5, function(i) mean(1-cr[,i], na.rm = TRUE)),5,1, dimnames = dnm.cr)[5],2)
  ...
  return(list(sauc.med.q13 = sauc.med.q13, succ = succ, cr = succ))
}
```
...
sauc <- data.frame(
	NN = c(N, rep(NA, 5)),
  Methods = c("HZ$_P$", "HZ$_O$","TNM$_P$", "TNM$_O$", "Proposed", "Succuess"),
  P1 = c(get(paste0("F", FD, ".N", N, ".D", ptdist, ".HR", HR, ".P", p1) )$sauc.med.q13, 
         get(paste0("F", FD, ".N", N, ".D", ptdist, ".HR", HR, ".P", p1) )$cr),
	
	P2 = c(get(paste0("F", FD, ".N", N, ".D", ptdist, ".HR", HR, ".P", p2) )$sauc.med.q13, 
         get(paste0("F", FD, ".N", N, ".D", ptdist, ".HR", HR, ".P", p2) )$cr),
	
	P3 = c(get(paste0("F", FD, ".N", N, ".D", ptdist, ".HR", HR, ".P", p3) )$sauc.med.q13, 
         get(paste0("F", FD, ".N", N, ".D", ptdist, ".HR", HR, ".P", p3) )$cr)
	)[c(1,2,5,6),]
...
output <- DT.MK1 %>%
  ...
  footnote(general = "
			...
			HZ$_O$ denotes the HZ model using the observed (published) studies.
