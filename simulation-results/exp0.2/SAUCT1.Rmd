...
sauc8 <- matrix(DATA[11,], ncol = 3, byrow = TRUE, 
...
sauc <- matrix(DATA[11,], ncol = 3, byrow = TRUE)
sauc6 <- matrix(DATA[11,], ncol = 3, byrow = TRUE, 
                dimnames = dimname.sauc)
...
sauc <- matrix(DATA[11,], ncol = 3, byrow = TRUE)
sauc4 <- matrix(DATA[11,], ncol = 3, byrow = TRUE, 
                dimnames = dimname.sauc)
...
sauc <- matrix(DATA[11,], ncol = 3, byrow = TRUE)
sauc2 <- matrix(DATA[11,], ncol = 3, byrow = TRUE, 
                dimnames = dimname.sauc)
...
p <- rep(c("0.8(25)", "0.6(33)", "0.4(50)", "0.2(100)"), each = 1000)                  
sauc.all1 <- cbind.data.frame(rbind.data.frame(sauc8, sauc6, sauc4, sauc2), p)
...
## N = 20
sauc.all1 %>% 
  mutate(p = factor(p , levels=c("0.8(25)", "0.6(33)", "0.4(50)", "0.2(100)"))) %>% 
  pivot_longer(cols = -p, names_to = "Model") %>% 
  ggplot(aes(x = p, y = value, fill = Model)) + 
  geom_boxplot(
    notch=TRUE, notchwidth = 0.5, outlier.shape = NA, alpha = 0.7) +
  scale_y_continuous(name = "SAUC", limits=c(0.6,0.9))+
