...
result_list <- function(path = "exp0.2/N20/ptdis2_HR1_p0.8.RData", 
  ...
  med <- apply(sauc.val, 2, function(x) median(x, na.rm = TRUE))*100 - apply(sauc.val, 2, function(x) median(x, na.rm = TRUE))[1]*100
...
tsauc.tab <- function(FD, N, ptdist, HR, p1=0.7, p2=0.5, p3=0.3){
	...
	sauc <- data.frame(
	NN = c(N, rep(NA, 4)),
 Methods = c("BNM$_P$", "BNM$_O$","TNM$_P$", "TNM$_O$", "Proposed"),
