...
for(marker.n in 1:5){
  ...
  a <- foreach(r=1:1000, .combine = "c", .packages = c("survival", "magrittr"))  %dorng%  {
    ...
    pdata <- .p.sim.logit(
        S=100,
        tK=set.ls$tK0,
        npt.range=set.ls$npt[[1]],  ## NUMBER OF PTS
...
#
save(alpha, file = "alpha-npt1-mis.RData")
...
for(Sn in 1:3){  ## i list  # Sample Size
  for(marker.no in 1:5){
    ...
    save(DATA, file = paste0("simipd_npt1mis_marker",marker.no ,"_S",Sn,".RData"))
  }}
...
.table.pars2 <- function(Sn, mark.n){
  ...
  load(paste0("simipd_npt1mis_marker",mark.n ,"_S",Sn,".RData"))
