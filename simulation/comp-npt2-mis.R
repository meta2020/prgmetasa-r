...
set.ls <- list(
  ...
  npt = list(c(50,100),
             c(50,200)),
  marker = list(c(x1.u = 0.7, x2.u = 0.3, x1.s = 0.1, x2.s = 0.4, v.sd = 0.14), ## HZ
...
#
save(alpha, file = "alpha-npt2-mis.RData")
...
# marker.n=1
# beta=3
simu.ipd <- function(S.n, npt.n=2, marker.n, beta=3){ 
...
for(Sn in 1:3){  ## i list  # Sample Size
  for(marker.no in 1:5){
    ...
    DATA <- foreach(r=1:1000, .combine = "cbind", .packages = c("survival", "magrittr","numDeriv"))  %dorng%  {
      ...
      simu.ipd(Sn, npt.n=2, marker.no, beta=3)
      # simu.ipd(1,1,1,3)
      
    }
    save(DATA, file = paste0("simipd_npt2mis_marker",marker.no ,"_S",Sn,".RData"))
  }}
...
.table.pars2 <- function(Sn, mark.n){
  ...
  load(paste0("simipd_npt2mis_marker",mark.n ,"_S",Sn,".RData"))
